import { ECSClient, RunTaskCommand } from '@aws-sdk/client-ecs';
import { EC2Client } from '@aws-sdk/client-ec2';
import { RDSClient, CreateDBInstanceCommand } from '@aws-sdk/client-rds';
import { Injectable, Logger } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { randomUUID } from 'crypto';
import { WebApp } from '../../entities/webapp.entity';
import { Instance } from '../../entities/instance.entity';
import { DatabaseConfig } from 'src/entities/database-config.entity';
import { DescribeInstancesCommand, RunInstancesCommand, waitUntilInstanceRunning } from '@aws-sdk/client-ec2';

@Injectable()
export class AwsProvisionerService {
    private readonly logger = new Logger(AwsProvisionerService.name);
    private readonly ecsClient: ECSClient;
    private readonly rdsClient: RDSClient;
    private readonly mockMode: boolean;
    private readonly ec2ClientsByRegion = new Map<string, EC2Client>();
    private readonly accessKeyId?: string;
    private readonly secretAccessKey?: string;

    constructor(private configService: ConfigService) {
        this.accessKeyId = configService.get<string>('aws.accessKeyId');
        this.secretAccessKey = configService.get<string>('aws.secretAccessKey');

        const awsConfig = {
            region: configService.get('aws.region'),
            ...(this.accessKeyId && this.secretAccessKey
                ? {
                    credentials: {
                        accessKeyId: this.accessKeyId,
                        secretAccessKey: this.secretAccessKey,
                    },
                }
                : {}),
        };
        this.ecsClient = new ECSClient(awsConfig);
        this.rdsClient = new RDSClient(awsConfig);
        this.mockMode = configService.get('aws.mockMode') !== 'false';
    }

    async provisionEcsTask(webapp: WebApp, instance: Instance): Promise<string> {
        if (this.mockMode) {
            // Return a realistic-looking mock ARN
            return `arn:aws:ecs:${webapp.region}:123456789012:task/kuberns-cluster/${randomUUID()}`;
        }

        const command = new RunTaskCommand({
            cluster: 'kuberns-cluster',
            taskDefinition: `kuberns-${webapp.framework}:latest`,
            launchType: 'FARGATE',
            overrides: {
                containerOverrides: [{
                    name: 'app',
                    environment: [
                        { name: 'APP_NAME', value: webapp.name },
                        { name: 'BRANCH', value: webapp.branch },
                    ],
                }],
            },
            networkConfiguration: { /* VPC config */ },
        });

        const response = await this.ecsClient.send(command);
        return response.tasks?.[0]?.taskArn ?? '';
    }

    async provisionRdsInstance(dbConfig: DatabaseConfig): Promise<string> {
        if (this.mockMode) {
            return `kuberns-db-${randomUUID().slice(0, 8)}.mock.rds.amazonaws.com`;
        }

        const command = new CreateDBInstanceCommand({
            DBInstanceIdentifier: `kuberns-${dbConfig.id}`,
            DBInstanceClass: 'db.t3.micro',
            Engine: dbConfig.dbType,
            MasterUsername: dbConfig.username,
            MasterUserPassword: dbConfig.password,
            AllocatedStorage: 20,
        });

        const response = await this.rdsClient.send(command);
        return response.DBInstance?.Endpoint?.Address ?? '';
    }

    async provisionEc2Instance(webapp: WebApp, instance: Instance): Promise<string> {
        const targetRegion = webapp.region || this.configService.get<string>('aws.region') || 'us-east-1';
        const subnetId = this.getRegionalEnvValue('AWS_SUBNET_ID', targetRegion)
            ?? this.configService.get<string>('aws.subnetId');
        const securityGroupId = this.getRegionalEnvValue('AWS_SECURITY_GROUP_ID', targetRegion)
            ?? this.configService.get<string>('aws.securityGroupId');

        if (this.mockMode) {
            const mockInstanceId = `i-mock${randomUUID().slice(0, 8)}`;
            const mockIp = `10.0.${Math.floor(Math.random() * 250) + 1}.${Math.floor(Math.random() * 250) + 1}`;
            this.logger.log(
                `AWS_MOCK_MODE enabled: ${mockInstanceId} treated as running in ${targetRegion} with IP ${mockIp} for app ${webapp.name}`,
            );
            return mockInstanceId;
        }

        if (!subnetId || !securityGroupId) {
            throw new Error('AWS_SUBNET_ID and AWS_SECURITY_GROUP_ID must be set when AWS_MOCK_MODE is false');
        }

        const amiId = this.getRegionalEnvValue('AWS_AMI_ID', targetRegion)
            ?? this.configService.get<string>('aws.amiId')
            ?? this.getDefaultAmi(targetRegion);
        const ec2Client = this.getEc2Client(targetRegion);
        this.logger.log(
            `Launching EC2 instance for app ${webapp.name} (webappId=${webapp.id}, instanceRowId=${instance.id}) in region ${targetRegion} using AMI ${amiId}`,
        );
        const command = new RunInstancesCommand({
            ImageId: amiId,
            InstanceType: 't3.micro',
            MinCount: 1,
            MaxCount: 1,
            SubnetId: subnetId,
            SecurityGroupIds: [securityGroupId],
            TagSpecifications: [{
                ResourceType: 'instance',
                Tags: [
                    { Key: 'Name', Value: `kuberns-${webapp.name}-${instance.id}` },
                    { Key: 'kuberns-webapp-id', Value: String(webapp.id) },
                ],
            }],
        });

        try {
            const response = await ec2Client.send(command);
            const instanceId = response.Instances?.[0]?.InstanceId;
            if (!instanceId) throw new Error('EC2 RunInstances returned no instance ID');

            const launchState = response.Instances?.[0]?.State?.Name ?? 'unknown';
            this.logger.log(`EC2 launch accepted for ${instanceId} with initial state=${launchState}`);

            const waiterResult = await waitUntilInstanceRunning(
                { client: ec2Client, maxWaitTime: 180 },
                { InstanceIds: [instanceId] },
            );
            if (waiterResult.state !== 'SUCCESS') {
                this.logger.warn(
                    `EC2 instance ${instanceId} did not reach running state within waiter window (state=${waiterResult.state})`,
                );
                return instanceId;
            }

            const describeResponse = await ec2Client.send(
                new DescribeInstancesCommand({ InstanceIds: [instanceId] }),
            );
            const awsInstance = describeResponse.Reservations?.[0]?.Instances?.[0];
            const publicIp = awsInstance?.PublicIpAddress ?? 'N/A';
            const privateIp = awsInstance?.PrivateIpAddress ?? 'N/A';

            this.logger.log(
                `EC2 instance ${instanceId} is running in ${targetRegion}. publicIp=${publicIp} privateIp=${privateIp}`,
            );
            return instanceId;
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            const stack = error instanceof Error ? error.stack : undefined;
            this.logger.error(`EC2 provisioning failed for app ${webapp.name} in ${targetRegion}: ${message}`, stack);
            throw error;
        }
    }

    private getDefaultAmi(region: string): string {
        const defaults: Record<string, string> = {
            'ap-south-1': 'ami-0f5ee99e8f549f4f6',
            'us-east-1': 'ami-002e81a9a522f1f19',
            'us-west-2': 'ami-0d705db840ec5f0c5',
            'eu-west-1': 'ami-0d71ea30463e0ff8d',
        };
        return defaults[region] ?? defaults['us-east-1'];
    }

    private getEc2Client(region: string): EC2Client {
        const cached = this.ec2ClientsByRegion.get(region);
        if (cached) return cached;

        const client = new EC2Client({
            region,
            ...(this.accessKeyId && this.secretAccessKey
                ? {
                    credentials: {
                        accessKeyId: this.accessKeyId,
                        secretAccessKey: this.secretAccessKey,
                    },
                }
                : {}),
        });
        this.ec2ClientsByRegion.set(region, client);
        return client;
    }

    private getRegionalEnvValue(prefix: string, region: string): string | undefined {
        const suffix = region.toUpperCase().replace(/-/g, '_');
        return process.env[`${prefix}_${suffix}`];
    }

}
